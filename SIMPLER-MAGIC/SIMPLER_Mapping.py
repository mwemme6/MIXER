...
class SIMPLER_Top_Data_Structure:
    ...
    def readoperations(self,bmfId):
        ...
        while isinstance(tline,str):
            ...
            stripped_tline = tline.strip()
            gate_type = stripped_tline.split(None, 1)[0] if stripped_tline else ''
            ...
            elif (gate_type.startswith("inv") or gate_type.startswith("nor") or gate_type.startswith("or")):
                op = 'inv1' if gate_type.startswith("inv") else gate_type
                remain = tline
                num_of_op = 0
                while (remain.find('.') != -1):
